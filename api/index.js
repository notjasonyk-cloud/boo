...
function getProducts(callback) {
  fetchProductsFromSellAuth((err, products) => {
    ...
    console.warn("SellAuth live API offline or 401, seamlessly using FALLBACK_PRODUCTS array");
  ...
  const req = https.request(options, (res) => {
    ...
    res.on('end', () => {
      try {
        ...
        if (res.statusCode === 200 && json && json.data) {
          cachedProducts = json.data;
          lastFetchTime = Date.now();
          callback(null, cachedProducts);
        } else {
          callback(new Error(`SellAuth API non-200 status code ${res.statusCode}`), null);
        }
      } catch (err) {
        callback(err, null);
      }
...
module.exports = (req, res) => {
  ...
  const cleanSlug = (slug || 'rust').toLowerCase();
  const targetPath = SLUG_TO_PATH[cleanSlug] || cleanSlug;
  const shortKey = cleanSlug.replace('-private', '').replace('-legends', '').replace('-raiders', '').replace('rainbow-six-siege', 'r6');
  const localAsset = PRODUCT_ASSETS[shortKey] || PRODUCT_ASSETS['rust'];

  getProducts((err, products) => {
    let liveProd = null;

    if (products && Array.isArray(products)) {
      liveProd = products.find(p => {
        const pPath = (p.path || '').toLowerCase();
        return pPath === targetPath || (p.aliases && p.aliases.includes(targetPath)) || (p.aliases && p.aliases.includes(cleanSlug));
      });

      if (!liveProd) {
        liveProd = products.find(p => (p.name || '').toLowerCase().includes(shortKey));
      }
    }
    ...
    if (!liveProd) {
      liveProd = {
        id: 774973,
        name: shortKey.toUpperCase() + " Cheat",
        description: localAsset.desc,
        currency: "USD",
        path: targetPath,
        variants: [
          { id: 1001, name: "1 Day", price: 7.49, stock: -1 },
          { id: 1002, name: "7 Days", price: 29.99, stock: -1 },
          { id: 1003, name: "30 Days", price: 59.99, stock: -1 }
        ]
      };
    }

    let prodImages = [];
    if (liveProd.images && Array.isArray(liveProd.images) && liveProd.images.length > 0) {
      prodImages = liveProd.images.map(i => (typeof i === 'object' && i.url) ? i.url : i);
    } else {
    ...
    fs.readFile(templatePath, 'utf8', (err, data) => {
      ...
      const productJson = {
        ...
        group_id: liveProd.group_id || null,
        category_id: null,
        category: null,
        type: "variant",
        visibility: "public",
        ...
        products_sold: liveProd.products_sold || 100,
        quantity_min: null,
        quantity_max: null,
        status_color: "#2ecc71",
      ...
      output = output.replace(/<meta property="og:image" content=".*?"/g, `<meta property="og:image" content="${prodImages[0]}"`);
      output = output.replace(/<meta name="twitter:image" content=".*?"/g, `<meta name="twitter:image" content="${prodImages[0]}"`);
